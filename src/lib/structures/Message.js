...
class Message extends Base {
    constructor(data, client) {
        ...
        this.reactions = {};
        if (data.author) {
            if (data.author.discriminator !== "0000") this.author = client.users.add(data.author, client);
            else this.author = new User(data.author, client);
        } else if (data.timestamp) this._client.emit("error", "MESSAGE_CREATE but no message author:\n" + JSON.stringify(data, null, 2));
        ...
        switch (this.type) {
            ...
            case 2: {
                if (this.author.id === data.mentions[0].id) data.content = `@${this.author.username} left the group.`;
                else data.content = `${this.author.mention} removed @${data.mentions[0].username}.`;
                break;
            }
            case 3: {
                if (data.call.ended_timestamp) {
                    ...
                    if (~data.call.participants.indexOf(client.user.id)) data.content = `You missed a call from ${this.author.mention}.`;
                    else data.content = `${this.author.mention} started a call.`;
                } else {
    ...
    update(data, client) {
        if (this.type === 3) (this.channel.call || this.channel.lastCall).update(data.call);
    ...
    get channelMentions() {
        if (this._channelMentions) return this._channelMentions;
    ...
    */
     ...
     * @arg {Number} [timeout] Time to wait until deleting the message.
     * @returns {Promise}
     */
